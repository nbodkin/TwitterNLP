...
def get_tweets():
    ...
    df = pandas.read_csv("tweets.csv", na_values={'user_verified':False}, dtype={'time':str, 'user_screen_name':str, 'text':str, 'tweet_type':str, 'hashtags':str, 'retweet_count':float, 'user_description':str, 'user_followers_count':float, 'user_friends_count':float, 'user_statuses_count':float, 'user_verified':bool})
    #original_tweets = pandas.read_csv("a.csv", header = None, delimiter="\t", quoting=csv.QUOTE_NONE, encoding='utf-8')
    df.to_dict('original_tweets')

    # list of dictionaries formatted like fetched_tweets = [{"text":"blah blah blah", "rt": 50, "username":"'}, {"text":"blah blah blah", "rt": 50, "username":"'}, ...]

    #original_tweets = [{'text': "I can’t believe you had the balls to tweet this. #LiarInChief #ImpeachTrumpNow", 'sentiment': 1}, {'text': "Secure our nations boarder!!!"}, {'text': "We all are waiting for you to go to jail"}, {'text': "The first contracts to support the construction of President Donald Trump's border wall are expected to be awarded this week using Pentagon funds"}, {'text': "Check this out - TRUTH!"}, {'text': "“What’s completely unacceptable is for Congesswoman Omar to target Jews, in this case Stephen Miller.” Jeff Ballabon, B2 Strategic, CEO.  @Varneyco"},
    ...
    for tweet in original_tweets:
        ...
        tweet['sentiment'], tweet['subjectivity'] = get_tweet_sentiment(
            tweet['text'])
